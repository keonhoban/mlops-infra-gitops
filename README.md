...
Production-grade ML Platform 설계/구현 프로젝트

이 프로젝트는 **GitOps(ArgoCD)** 를 운영 중심축으로 삼아,
**모델 학습 → 등록 → 검증 → 배포 → 트래픽 전환 → 실패 시 롤백**까지를
**재현 가능하게 자동화**한 ML Platform입니다.

핵심 키워드:

- **Separation**: dev/prod, Core/Baseline/Optional 경계가 구조적으로 분리됨
- **Automation**: Airflow → MLflow → Triton → FastAPI E2E 루프 자동화
- **Proof**: 토글/경계/헬스/E2E 상태를 스냅샷으로 남겨 “말이 아닌 증거”로 제시

---

## TL;DR

- **Core**: Airflow가 모델 생명주기(학습/등록/검증/배포/롤백)를 통제하고, Triton/FastAPI가 런타임 전환(Load/Ready/Reload)을 수행합니다.
...
- **Optional**: Feature Store(Feast 등)는 **Attach/Detach** 하며, OFF 시에는 “비파괴 Detach”로 경계를 유지합니다.  
  (실수 방지를 위해 **Manual Sync = 의도적 토글**로 운용)
...
- Triton Inference Server (Serving Runtime)
- FastAPI (Alias A/B 라우팅 + Reload API + Metrics)
...
2) READY Sensor 통과  
3) Triton Load 성공  
4) (가능한 경우) Triton Smoke Test(Inference) 성공  
5) FastAPI Reload 성공  
6) Slack 알림 전송 확인  

흐름 정의: `docs/overview/e2e-flow.md`

> Note:  
> 현재 `docs/proof/latest/e2e_success/`는 “서빙 런타임 증거(Triton/FastAPI/Reload)”를 파일로 증명합니다.  
> MLflow/READY/Slack을 포함한 “파이프라인 전체 증거”는 Airflow proof 확장으로 동일 방식으로 추가 가능합니다.
...
- Core 헬스 상태 확인 포함  
경로: `docs/proof/latest/core_only/`

### 2) Optional ON 증명
- optional-envs-dev/prod + feast-dev/prod attach 성공 로그 포함  
경로: `docs/proof/latest/optional_on/`

### 3) E2E Success (Serving Runtime)
- Triton READY + repository index에서 READY 모델 확인
- FastAPI `/health`, `/models`, `/metrics` 스냅샷
- FastAPI reload 성공(JSON)  
경로: `docs/proof/latest/e2e_success/`

---

## Bootstrap Flow (ArgoCD 진입점)

이 리포지토리는 “루트 → 레이어(AppSet)” 구조로 부팅됩니다.

1) `bootstrap/` : ArgoCD에 Root 앱들을 생성하는 진입점  
2) `root-apps` : AppProject / namespaces / Core AppSet 생성  
3) `root-baseline` : Baseline(AppSet) 생성 (Always-on)  
4) `root-optional` : Optional 앱(Feast 등) 생성/제거 (Manual = 토글)

이 구조는 Root → Layer(AppSet) 계층을 명확히 하여,
Core/Baseline/Optional 책임을 분리하고,
Optional 토글이 Core/Baseline에 영향을 주지 않도록 설계되었습니다.
...
- E2E(서빙 런타임) 증명: `make proof-e2e` (또는 `./ops/proof/proof_e2e_success.sh`)
- GitOps 상태 확인: `argocd app list`

### 3분 데모 루트 (면접용)
1) `make proof-core` → Optional OFF + Core health 증거 확인  
2) `make optional-on` → Optional attach 증거 확인  
3) `make optional-off` → scope=0 복귀 증거 확인  
4) `make proof-e2e` → Triton/FastAPI/Reload 증거 확인  
- 증거 인덱스: `docs/proof/latest/INDEX.md`

### 10분 심화 데모 루트
- `docs/overview/e2e-flow.md`로 성공 조건/의존성(스토리지/런타임) 설명
- `docs/overview/argocd-boundary.md`로 dev/prod + Optional guardrail 설명
- `docs/security/secrets.md`로 SealedSecrets 운영 원칙/DR 설명
