...
#### Feature Store: 현재 Redis → 실 서비스 ScyllaDB 전환 경로

현재 온라인 스토어는 Redis(in-memory key-value)로 구성되어 있습니다.
대규모 사용자 feature lookup(수천만 user_id, wide-column 시계열 feature)이 필요한 실 서비스 환경에서는
ScyllaDB(Cassandra-compatible wide-column store, 수평 확장 지원)로 전환이 필요합니다.

전환 시 변경 포인트:
1. `feast_repo/feature_store.yaml`의 `online_store.type: redis` → `type: cassandra` (ScyllaDB Cassandra 호환 드라이버 사용)
2. Optional Helm chart에 ScyllaDB StatefulSet 추가 (현재 Redis Deployment 대체)
3. Airflow feature materialize Task의 store write 경로 재연결 (Feast SDK API 수준 변경)
