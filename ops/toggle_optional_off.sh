...
log "[OFF] delete optional ArgoCD Applications/ApplicationSets by label (best-effort)"
run "20_delete_optional_apps_by_label"    kubectl -n argocd delete application -l scope=optional --ignore-not-found=true || true
...
log "[OFF] proof"
run "90_after_argocd_apps"    kubectl -n argocd get applications.argoproj.io -o wide
